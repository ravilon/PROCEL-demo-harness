...
O HTTP publisher escuta em `http://localhost:3000`. O Mosquitto escuta em `localhost:1883` no host; os containers se comunicam pela rede Docker. As portas sao vinculadas ao loopback do servidor. O `.env` deste projeto pode conter `PROCEL_MQTT_HOST` para o console antigo, mas o HTTP publisher usa `MQTT_URL=mqtt://mqtt:1883` dentro do Compose.
